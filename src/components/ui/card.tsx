...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "overflow-hidden rounded-[18px] border border-white/70 bg-white/78 shadow-[0_14px_36px_rgba(15,23,42,0.06)] backdrop-blur-md sm:rounded-[24px]",
        className
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("text-lg font-semibold leading-tight tracking-[-0.03em] text-slate-950 sm:text-xl", className)} {...props} />
  )
);
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-sm leading-5 text-slate-500", className)} {...props} />
  )
);
